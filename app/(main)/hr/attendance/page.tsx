...
function MarkModal({ emp, day, onClose, onSave }: { emp:EmpAtt;day:number;onClose:()=>void;onSave:(d:{clockIn:string;clockOut:string})=>void }) {
  ...
  const Toggle = ({ on, toggle }: { on:boolean; toggle:()=>void }) => (
    <div onClick={toggle} style={{ width:36,height:20,borderRadius:10,background:on?"#4f46e5":"#d1d5db",cursor:"pointer",position:"relative",transition:"background 0.2s",flexShrink:0 }}>
      ...
      <div style={{ background:"#fff",borderRadius:16,width:"min(540px,95vw)",boxShadow:"0 20px 60px rgba(0,0,0,0.2)",overflow:"hidden" }} onClick={e => e.stopPropagation()}>
        <div style={{ padding:"16px 20px",borderBottom:"1px solid #f3f4f6",display:"flex",alignItems:"center",justifyContent:"space-between" }}>
          ...
          <div style={{ display:"flex",alignItems:"center",gap:10 }}>
            <span style={{ fontSize:11,fontWeight:700,color:"#4f46e5",background:"#ede9fe",padding:"3px 10px",borderRadius:6 }}>Mark Attendance</span>
        ...
        <div style={{ padding:"12px 20px",borderTop:"1px solid #f3f4f6",display:"flex",justifyContent:"flex-end",gap:8 }}>
          ...
          <button onClick={()=>onSave({clockIn,clockOut})} style={{ padding:"8px 24px",borderRadius:8,border:"none",background:"linear-gradient(135deg,#4f46e5,#7c3aed)",color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",gap:6 }}><i className="ri-check-line"/> Save</button>
        </div>
      </div>
    </div>
  );
        ...
        <div style={{ background:"#fff",borderRadius:16,width:"min(780px,95vw)",maxHeight:"90vh",overflow:"auto",boxShadow:"0 20px 60px rgba(0,0,0,0.18)" }} onClick={e => e.stopPropagation()}>
          <div style={{ padding:"16px 24px",borderBottom:"1px solid #f3f4f6",display:"flex",alignItems:"center",justifyContent:"space-between" }}>
            <div style={{ display:"flex",alignItems:"center",gap:12 }}>
              ...
              <div>
                <div style={{ display:"flex",alignItems:"center",gap:8 }}><span style={{ fontWeight:800,fontSize:16,color:"#1e1b4b" }}>{emp.name}</span><span style={{ fontSize:10,fontWeight:700,color:"#4f46e5",background:"#ede9fe",padding:"2px 8px",borderRadius:20 }}>{emp.role}</span></div>
              ...
              <div style={{ display:"flex",flexDirection:"column" as const,gap:8 }}>
                <div style={{ background:"#f8f9fa",borderRadius:10,padding:"12px 14px",border:"1px solid #f3f4f6" }}>
                  <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6 }}>
                    <div style={{ display:"flex",alignItems:"center",gap:6 }}>
                      <span style={{ width:10,height:10,borderRadius:"50%",background:"#4f46e5",boxShadow:"0 0 0 2px rgba(79,70,229,0.3)",display:"inline-block",flexShrink:0 }}/>
          ...
          <thead>
            <tr style={{ background:"#f5f3ff" }}>
              ...
              {Array.from({length:days},(_,i)=>i+1).map(d => {
                ...
                return <th key={d} style={{ ...TH,minWidth:34,background:isToday?"#ede9fe":isHol?"#fef9c3":isSun?"#f9fafb":"#f5f3ff",color:isSun?"#9ca3af":isHol?"#ca8a04":isToday?"#4f46e5":"#6b7280" }}><div style={{ fontSize:9,fontWeight:500 }}>{DAY_SHORT[(3+d-1)%7]}</div><div style={{ fontSize:12,fontWeight:700 }}>{d}</div></th>;
              })}
          ...
          <tbody>
            {filtered.map((emp,idx) => (
              <tr key={emp.id} style={{ borderTop:"1px solid #f3f4f6",background:idx%2===0?"#fff":"#fafafa" }}>
                ...
                <td style={{ padding:"8px",textAlign:"center",fontWeight:700,color:countP(emp)>0?"#16a34a":"#dc2626",background:"#f5f3ff" }}>{countP(emp)}/{workDays}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function ByMemberTab({ month, year }: { month:number;year:number }) {
  ...
  const THs: React.CSSProperties = { padding:"10px 14px",fontWeight:700,fontSize:11,color:"#6b7280",textTransform:"uppercase" as const,letterSpacing:"0.04em",borderBottom:"2px solid #ede9fe",whiteSpace:"nowrap",background:"#f9f9fb" };
  ...
  return (
    <>
      ...
      <div className="row g-3" style={{ marginBottom:"1.25rem" }}>
        {[{label:"Working Days",value:summary.working,color:"#4f46e5",bg:"#ede9fe",icon:"ri-calendar-2-line"},{label:"Present",value:summary.present,color:"#16a34a",bg:"#dcfce7",icon:"ri-checkbox-circle-line"},{label:"Late",value:summary.late,color:"#db2777",bg:"#fce7f3",icon:"ri-time-line"},{label:"Half Day",value:summary.halfDay,color:"#ea580c",bg:"#fed7aa",icon:"ri-contrast-line"},{label:"Absent",value:summary.absent,color:"#dc2626",bg:"#fee2e2",icon:"ri-close-circle-line"},{label:"Holidays",value:summary.holiday,color:"#ca8a04",bg:"#fef9c3",icon:"ri-star-line"}].map(c=>(
          <div key={c.label} className="col">
            <div style={{ background:"#fff",borderRadius:12,border:"1px solid #ede9fe",padding:"12px",textAlign:"center",boxShadow:"0 2px 8px rgba(79,70,229,0.06)" }}>
            ...
            {Array.from({length:days},(_,i)=>days-i).map(d => {
              ...
              return (
                <tr key={d} style={{ background:d%2===0?"#fafafa":"#fff" }}>
                  ...
                  <td style={{ ...TDs,textAlign:"center" }}>{showClock&&<button style={{ fontSize:12,padding:"3px 10px",borderRadius:6,border:"1px solid #ede9fe",background:"#fafafa",color:"#4f46e5",cursor:"pointer",fontWeight:600 }}><i className="ri-search-line"/> Details</button>}{!showClock&&<span style={{ color:"#d1d5db" }}>—</span>}</td>
                </tr>
              );
            })}
          </tbody>
        ...
        <table style={{ borderCollapse:"collapse",minWidth:"100%",fontSize:12 }}>
          <thead>
            <tr style={{ background:"#f5f3ff" }}>
              ...
              {Array.from({length:days},(_,i)=>i+1).map(d => {
                const isSun=SUNDAYS.has(d),isHol=HOLIDAYS.has(d),isToday=d===TODAY;
                return <th key={d} style={{ ...TH,minWidth:40,background:isToday?"#ede9fe":isHol?"#fef9c3":isSun?"#f9fafb":"#f5f3ff",color:isSun?"#9ca3af":isHol?"#ca8a04":isToday?"#4f46e5":"#6b7280" }}><div style={{ fontSize:9,fontWeight:500 }}>{DAY_SHORT[(3+d-1)%7]}</div><div style={{ fontSize:12,fontWeight:700 }}>{d}</div></th>;
              })}
            ...
            {filtered.map((emp,idx) => {
              ...
              return (
                <tr key={emp.id} style={{ borderTop:"1px solid #f3f4f6",background:idx%2===0?"#fff":"#fafafa" }}>
                  ...
                  <td style={{ padding:"8px",textAlign:"center",fontWeight:700,color:totalH>0?"#4f46e5":"#9ca3af",background:"#f5f3ff" }}>{totalH>0?fmtHs(totalH):"—"}</td>
                </tr>
              );
            })}
          </tbody>
...
function ByLocationTab({ filtered }: { filtered: EmpAtt[] }) {
  ...
  return (
    <div>
          ...
          <div key={i} className="col-md-4">
            <div style={{ background:"#fff",borderRadius:12,border:"1px solid #ede9fe",padding:"16px",boxShadow:"0 2px 8px rgba(79,70,229,0.06)" }}>
              <div style={{ display:"flex",alignItems:"flex-start",gap:12 }}>
                <div style={{ width:40,height:40,borderRadius:10,background:i===2?"#f3f4f6":"#ede9fe",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0 }}><i className={o.icon} style={{ fontSize:20,color:i===2?"#6b7280":"#4f46e5" }}/></div>
      ...
      <div style={{ background:"#fff",borderRadius:12,border:"1px solid #ede9fe",overflow:"hidden",boxShadow:"0 2px 12px rgba(79,70,229,0.06)" }}>
        <div style={{ padding:"14px 18px",borderBottom:"1px solid #f3f4f6" }}><span style={{ fontWeight:700,fontSize:14,color:"#1e1b4b" }}><i className="ri-map-pin-2-line" style={{ marginRight:6,color:"#4f46e5" }}/> Attendance by Location — Today</span></div>
        <div style={{ background:"#f5f3ff",minHeight:220,display:"flex",flexDirection:"column" as const,alignItems:"center",justifyContent:"center",gap:12,padding:"2rem" }}>
        ...
        <div style={{ display:"flex",gap:2 }}>
          {TABS.map(t => (
            <button key={t.id} onClick={()=>setTab(t.id)}
              style={{ display:"flex",alignItems:"center",gap:6,padding:"8px 14px",borderRadius:8,border:"none",cursor:"pointer",fontSize:13,fontWeight:tab===t.id?700:500,background:tab===t.id?"#ede9fe":"transparent",color:tab===t.id?"#4f46e5":"#6b7280" }}>
