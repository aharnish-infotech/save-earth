...
import { AUDITS, STATUS_STYLE, AUDIT_BANKS, AUDIT_AUDITORS } from "@/lib/data/audits";
...
export default function CompletedPage() {
  const base = AUDITS.filter(a => a.status === "Completed");
  ...
  const readyCount = base.length;

  return (
    <div style={{ padding:"24px 0" }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:4 }}>
        <h4 style={{ fontSize:22, fontWeight:800, color:"#111827", margin:0 }}>Completed Audits</h4>
      </div>
      <div style={{ fontSize:12, color:"#9ca3af", marginBottom:16 }}>Dashboard / Audit Operations / <span style={{ color:"#16a34a", fontWeight:600 }}>Completed</span></div>

      {/* Workflow Banner */}
      <div style={{ background:"#eff6ff", border:"1px solid #bfdbfe", borderRadius:10, padding:"12px 18px", display:"flex", alignItems:"center", gap:10, marginBottom:20 }}>
        <i className="ri-information-line" style={{ color:"#2563eb", fontSize:17 }}/>
        <span style={{ fontSize:12, color:"#1d4ed8", fontWeight:500 }}>
          <strong>Step 2 — Coordinator Review:</strong> Auditor has submitted. Coordinator should review and move to Pending Review.
        </span>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:14, marginBottom:20 }}>
        {[
          { label:"Completed",         value:base.length,   color:"#0891b2", bg:"#ecfeff",  icon:"ri-check-line",           border:"#0891b2" },
          { label:"Ready for Review",  value:readyCount,    color:"#7c3aed", bg:"#f5f3ff",  icon:"ri-eye-line",             border:"#7c3aed" },
          { label:"Avg. Score",        value:`${avgScore}%`,color:"#16a34a", bg:"#f0fdf4",  icon:"ri-bar-chart-2-line",     border:"#16a34a" },
          { label:"Total Photos",      value:base.reduce((s,a)=>s+a.photos,0), color:"#ca8a04", bg:"#fefce8", icon:"ri-image-2-line", border:"#ca8a04" },
        ].map(c => (
        ...
        <div style={{ overflowX:"auto" }}>
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            <thead><tr>
              ...
              <th style={{ ...TH, textAlign:"center" }}>SUBMITTED</th>
              <th style={{ ...TH, textAlign:"center" }}>ACTION</th>
            </tr></thead>
            <tbody>
              {paged.length === 0 ? (
                <tr><td colSpan={9} style={{ padding:"60px 24px", textAlign:"center", color:"#9ca3af" }}>
                  <i className="ri-check-line" style={{ fontSize:36, display:"block", marginBottom:8, opacity:0.3 }}/>No completed audits
                </td></tr>
              ) : paged.map(a => (
                <tr key={a.id} onMouseEnter={e=>e.currentTarget.style.background="#f9fafb"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                  <td style={TD}>
                    <span style={{ fontSize:11, fontWeight:700, color:"#0891b2", background:"#ecfeff", borderRadius:6, padding:"3px 10px", fontFamily:"monospace" }}>{a.id}</span>
                  </td>
                  <td style={TD}>
                  ...
                  <td style={{ ...TD, textAlign:"center", color:"#6b7280", fontSize:12 }}>{a.submittedDate || "—"}</td>
                  <td style={{ ...TD, textAlign:"center" }}>
                    <div style={{ display:"flex", gap:6, justifyContent:"center" }}>
                      ...
                      <button title="Send for Review" style={{ padding:"5px 10px", borderRadius:7, border:"none", background:"#7c3aed", color:"#fff", cursor:"pointer", fontSize:11, fontWeight:700, display:"flex", alignItems:"center", gap:4 }}><i className="ri-send-plane-line" style={{ fontSize:12 }}/>Review</button>
                    </div>
                  </td>
                </tr>
              ))}
