...
export default function DeliveredPage() {
  const base = AUDITS.filter(a => a.status === "Delivered");
  ...
  return (
    <div style={{ padding:"24px 0" }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:4 }}>
        <h4 style={{ fontSize:22, fontWeight:800, color:"#111827", margin:0 }}>Delivered Audits</h4>
      </div>
      <div style={{ fontSize:12, color:"#9ca3af", marginBottom:20 }}>Dashboard / Audit Operations / <span style={{ color:"#16a34a", fontWeight:600 }}>Delivered</span></div>

      {/* Stats */}
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:14, marginBottom:20 }}>
        {[
          { label:"Delivered",   value:base.length,    color:"#059669", bg:"#d1fae5", icon:"ri-send-plane-line",       border:"#059669" },
        ...
        <div style={{ overflowX:"auto" }}>
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            <thead><tr>
              ...
              <th style={{ ...TH, textAlign:"center" }}>DELIVERED ON</th>
              <th style={{ ...TH, textAlign:"center" }}>ACTION</th>
            </tr></thead>
            <tbody>
              {paged.length === 0 ? (
                <tr><td colSpan={9} style={{ padding:"60px 24px", textAlign:"center", color:"#9ca3af" }}>
                  <i className="ri-send-plane-line" style={{ fontSize:36, display:"block", marginBottom:8, opacity:0.3 }}/>No delivered audits
                </td></tr>
              ) : paged.map(a => (
                <tr key={a.id} onMouseEnter={e=>e.currentTarget.style.background="#f9fafb"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                  <td style={TD}>
                    <span style={{ fontSize:11, fontWeight:700, color:"#059669", background:"#d1fae5", borderRadius:6, padding:"3px 10px", fontFamily:"monospace" }}>{a.id}</span>
                  </td>
                  <td style={TD}>
                  ...
                  <td style={{ ...TD, textAlign:"center", color:"#6b7280", fontSize:12 }}>{a.deliveredDate || "—"}</td>
                  <td style={{ ...TD, textAlign:"center" }}>
                    <div style={{ display:"flex", gap:6, justifyContent:"center" }}>
                      ...
                      <button title="Download PDF" style={{ padding:"5px 10px", borderRadius:7, border:"1px solid #e5e7eb", background:"#fff", color:"#374151", cursor:"pointer", fontSize:11, fontWeight:700, display:"flex", alignItems:"center", gap:4 }}><i className="ri-download-line" style={{ fontSize:12 }}/>PDF</button>
                      <button title="Share" style={{ width:30, height:30, borderRadius:7, border:"1px solid #e5e7eb", background:"transparent", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", color:"#2563eb" }}><i className="ri-share-line" style={{ fontSize:14 }}/></button>
                    </div>
                  </td>
                </tr>
              ))}
