...
import { AUDITS, AUDIT_BANKS, AUDIT_AUDITORS } from "@/lib/data/audits";
...
export default function PendingReviewPage() {
  const base = AUDITS.filter(a => a.status === "Pending Review");
  ...
  return (
    <div style={{ padding:"24px 0" }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:4 }}>
        <h4 style={{ fontSize:22, fontWeight:800, color:"#111827", margin:0 }}>Pending Review</h4>
      </div>
      <div style={{ fontSize:12, color:"#9ca3af", marginBottom:16 }}>Dashboard / Audit Operations / <span style={{ color:"#16a34a", fontWeight:600 }}>Pending Review</span></div>

      {/* Workflow Banner */}
      <div style={{ background:"#fffbeb", border:"1px solid #fde68a", borderRadius:10, padding:"12px 18px", display:"flex", alignItems:"center", gap:10, marginBottom:20 }}>
        <i className="ri-time-line" style={{ color:"#d97706", fontSize:17 }}/>
        <span style={{ fontSize:12, color:"#92400e", fontWeight:500 }}>
          <strong>Step 3 — Admin Review:</strong> Coordinator has submitted for review. Admin should review scores, photos, and NCRs before approving.
        </span>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:14, marginBottom:20 }}>
        {[
          { label:"Pending Review",  value:base.length,    color:"#ca8a04", bg:"#fefce8", icon:"ri-time-line",           border:"#ca8a04" },
          { label:"Avg. Score",      value:`${avgScore}%`, color:"#0891b2", bg:"#ecfeff", icon:"ri-bar-chart-2-line",    border:"#0891b2" },
          { label:"Total NCRs",      value:base.reduce((s,a)=>s+a.ncr,0), color:"#dc2626", bg:"#fef2f2", icon:"ri-alert-line", border:"#dc2626" },
          { label:"Total Photos",    value:base.reduce((s,a)=>s+a.photos,0), color:"#7c3aed", bg:"#f5f3ff", icon:"ri-image-2-line", border:"#7c3aed" },
        ].map(c => (
            ...
            <tbody>
              {paged.length === 0 ? (
                <tr><td colSpan={9} style={{ padding:"60px 24px", textAlign:"center", color:"#9ca3af" }}>
                  <i className="ri-time-line" style={{ fontSize:36, display:"block", marginBottom:8, opacity:0.3 }}/>No audits pending review
                </td></tr>
              ) : paged.map(a => (
                <tr key={a.id} onMouseEnter={e=>e.currentTarget.style.background="#f9fafb"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                  <td style={TD}>
                    <span style={{ fontSize:11, fontWeight:700, color:"#ca8a04", background:"#fef9c3", borderRadius:6, padding:"3px 10px", fontFamily:"monospace" }}>{a.id}</span>
                  </td>
                  <td style={TD}>
                  ...
                  <td style={{ ...TD, textAlign:"center" }}>
                    {a.score !== undefined ? (
                      <span style={{ fontSize:12, fontWeight:800, color:a.score>=90?"#16a34a":a.score>=75?"#ca8a04":"#dc2626", background:a.score>=90?"#dcfce7":a.score>=75?"#fef9c3":"#fee2e2", borderRadius:6, padding:"3px 10px" }}>{a.score}%</span>
                  ...
                  <td style={{ ...TD, textAlign:"center", color:"#6b7280", fontSize:12 }}>{a.reviewedDate || "—"}</td>
                  <td style={{ ...TD, textAlign:"center" }}>
                    <div style={{ display:"flex", gap:6, justifyContent:"center" }}>
                      ...
                      <button title="Approve" style={{ padding:"5px 10px", borderRadius:7, border:"none", background:"#16a34a", color:"#fff", cursor:"pointer", fontSize:11, fontWeight:700, display:"flex", alignItems:"center", gap:4 }}><i className="ri-checkbox-circle-line" style={{ fontSize:12 }}/>Approve</button>
                      <button title="Return" style={{ padding:"5px 10px", borderRadius:7, border:"1px solid #e5e7eb", background:"#fff", color:"#dc2626", cursor:"pointer", fontSize:11, fontWeight:700, display:"flex", alignItems:"center", gap:4 }}><i className="ri-arrow-go-back-line" style={{ fontSize:12 }}/>Return</button>
                    </div>
                  </td>
                </tr>
              ))}
