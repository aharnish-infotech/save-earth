...
import { COUNSELOR_LEADERBOARD } from "@/lib/constants/dashboard-data";

const COLORS = ["#7c3aed","#2563eb","#10b981","#d97706","#ec4899"];
const INITIALS = ["RS","AV","NS","SP","RY"];

export default function CounselorLeaderboard() {
  return (
    <div className="card custom-card h-100 mb-0">
      <div className="card-header zf-widget-header">
        <h3 className="zf-widget-title">Counselor Leaderboard</h3>
        <Link href="#" className="zf-view-all">View All <i className="ri-arrow-right-s-line" /></Link>
      </div>
      <div className="card-body zf-widget-body">
        {COUNSELOR_LEADERBOARD.map((c, i) => (
          <div key={c.rank} className="zf-lb-item">
            ...
            <span className="zf-lb-count">{c.count} Admissions {c.trophy && "🏆"}</span>
          </div>
        ))}
      </div>
    </div>
